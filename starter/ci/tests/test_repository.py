...
# ---------------------------------------------------------------------------
# Die Luecke aus dem Coverage-Lauf
# ---------------------------------------------------------------------------
# ``pytest --cov=shop --cov-branch --cov-report=term-missing`` meldete fuer
# repository.py eine fehlende Zeile und einen halb genommenen Zweig: den
# Rueckgabewert ``None`` in ``get()``. Die Zeile war nicht nur ungetestet,
# sie war eine echte fachliche Zusage: unbekannte ID wirft nicht, sondern
# liefert ``None``. Genau dafuer ist Coverage gut, als Suchlicht.
